<template>
    <div>
        <div class='shopcar'>
            ...
            <router-link to='/Home/tuijian/2'>去首页逛逛</router-link>
        </div>
        <div class='title'>
            大家都在买的宝贝
        </div>
        <shopcar :list=list></shopcar>
    </div>
</template>
<script>
...
export default {
    ...
    methods:{
        getdata:function(){
            this.$http.get('/api/shop').then(function(res){
                this.list=res.data.data.extra.Suggest
                console.log(res.data.data.extra.Suggest)
            })
        }
    },
    mounted () {
